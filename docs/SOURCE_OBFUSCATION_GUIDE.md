...
Hệ thống sẽ thực hiện làm rối, mã hóa toàn bộ code và xuất ra tệp tin phân phối hoàn chỉnh: `extensions/novel/hieu05_truyencv/plugin.zip`.
...
PAYLOAD = SALT + SOURCE + AUTHOR
        = "com.vbook.app" + "https://truyencv.io" + "hieu05"
...
```bash
npm run encrypt:obfuscate -- extensions/novel/hieu05_truyencv
```
...
```bash
npm run encrypt:aes -- extensions/novel/hieu05_truyencv
```
...
```bash
npm run build:ext -- --plugin extensions/novel/hieu05_truyencv/plugin.json
```
...
```bash
node tools/cli/vbook_decoder.js extensions/novel/hieu05_truyencv/plugin.zip
...
```bash
npm run encrypt:pipeline -- extensions/novel/hieu05_truyencv
```
