...
// POST request với headers và body (hỗ trợ: GET, POST, PUT, DELETE, PATCH)
let response = fetch(url, {
    method: "POST",
    headers: {
...
let b64 = response.base64();            // Base64 encoded string
let b64 = response.base64("utf-8");     // Base64 với charset
...
// NOTE: Http.get() cũng hỗ trợ .params() (alias của .queries())

request.body("raw string data");        // Raw body data (chỉ POST)

request.timeout(10000);                 // Timeout (ms)

// Execute requests
let doc = request.html();               // Execute và parse HTML
let doc = request.html("utf-8");        // HTML với charset
let text = request.string();            // Execute và lấy string
let text = request.string("utf-8");     // String với charset
let url = request.url();                // Lấy URL của request
```
...
// Load HTML trực tiếp (không cần URL)
browser.loadHtml(htmlString, timeout);  // Load HTML string vào browser

// Lấy biến JavaScript từ context của trang
let value = browser.getVariable("variableName"); // Lấy giá trị biến JS

// Chặn các URL không mong muốn (quảng cáo, tracking,...)
browser.block(["ads.com", "tracking.js"]); // Chặn load các URL chứa pattern
...
Selector sử dụng cú pháp [jsoup](https://jsoup.org/cookbook/extracting-data/selector-syntax)
...
let attrs = elements.first().attributes();     // Tất cả attributes (JSON)

// Lấy HTML
let innerHTML = elements.first().html();       // Inner HTML

// Thuộc tính và sub-select
let count = elements.length;                   // Số lượng elements (property)
let subElements = elements.select("span");     // Select con trong tập elements
...
let names = chapters.map(element => {
    return element.text();
});
    ...
    return Response.success([
        {
            ...
            script: "gen.js"                  // Script xử lý (sẽ gọi gen.js)
        },
        {
            title: "Truyện hot", 
...
// NOTE: Kết quả home.js sẽ gọi sang script được chỉ định (gen.js)
// với url = input, page = <rỗng>
// Script đó trả về: [{name, link, host, cover, description}], next
// Nếu next != null, app tiếp tục gọi script với page = next
// Nếu next = null hoặc rỗng → dừng load
```
...
## SEARCH.JS - Tìm kiếm:
NOTE: Nếu không có search script, app sẽ tự dùng Google Search.
...
// COMMENT.JS - Script bình luận (tùy chọn):
...
// description: Thông tin thêm (thời gian, etc.)
```
...
## Canvas và Image:
```javascript
// Tạo canvas với kích thước
let canvas = Graphics.createCanvas(width, height);

// Tạo image từ dữ liệu
let image = Graphics.createImage(imageData);

// Thuộc tính của Image
let w = image.width;                    // Chiều rộng
let h = image.height;                   // Chiều cao

// Vẽ image lên canvas
canvas.drawImage(image, x, y);                              // Vẽ ảnh tại vị trí (x, y)
canvas.drawImage(image, x, y, w, h, extra);                 // Vẽ với kích thước
canvas.drawImage(image, sx, sy, sw, sh, dx, dy, dw, dh);    // Vẽ với crop

// Xuất kết quả
let result = canvas.capture();           // Chụp canvas thành ảnh
```

## Ví dụ: Xử lý CAPTCHA bằng canvas:
```javascript
function solveCaptcha(imageUrl) {
    let img = Graphics.createImage(fetch(imageUrl).base64());
    let canvas = Graphics.createCanvas(img.width, img.height);
    canvas.drawImage(img, 0, 0);
    let captchaData = canvas.capture();
    // Xử lý captchaData...
    return captchaData;
}
```
...
=====================================
# VIII. COOKIE VÀ LOCALCOOKIE
=====================================

## localCookie - Cookie cục bộ:
```javascript
// Đặt cookie
localCookie.setCookie("session=abc123;token=xyz");
...
let ws = WebSocket(url, options);
// url: WebSocket URL (ws:// hoặc wss://)
// options: object cấu hình (headers, etc.)
```

=====================================
# X. CÁC HÀM TIỆN ÍCH VÀ LƯU Ý ĐẶC BIỆT
=====================================
...
## Console và debug:
```javascript
// Console.log / console.log / Log.log đều tương đương
...
=====================================
# XII. EXTENSION DỊCH THUẬT (TRANSLATE)
=====================================
...
=====================================
# XIII. EXTENSION TEXT-TO-SPEECH (TTS)
=====================================
...
```
=====================================
# XVI. USER AGENT VÀ BROWSER SETTINGS
=====================================

## Các loại User Agent có sẵn:
```javascript
// UserAgent cơ bản - Các user agent được định sẵn
browser.setUserAgent(UserAgent.system());      // System user agent
browser.setUserAgent(UserAgent.chrome());      // Chrome user agent
browser.setUserAgent(UserAgent.android());     // Android user agent
browser.setUserAgent(UserAgent.ios());         // iOS user agent
...
=====================================
# XVII. LOCALSTORAGE VÀ CACHESTORAGE
=====================================
...
=====================================
# XVIII. COOKIE HANDLING CHI TIẾT
=====================================
...
=====================================
# XX. SCRIPT EXECUTION VÀ DYNAMIC CODE
=====================================
...
=====================================
# XXI. ARRAY VÀ STRING PROCESSING NÂNG CAO
=====================================
...
=====================================
# XXII. ERROR HANDLING VÀ DEBUGGING NÂNG CAO
=====================================
...
=====================================
# XXIII. CONFIG VÀ ENVIRONMENT MANAGEMENT
=====================================
...
=====================================
# XXII-b. LƯU Ý QUAN TRỌNG VÀ BEST PRACTICES
=====================================
...
=====================================
# XXIV. TEMPLATE PATTERNS THƯỜNG DÙNG
=====================================
...
```
=====================================
# XXV. KẾT LUẬN VÀ BEST PRACTICES
=====================================
...
- Cài [vBook Extension Maker](https://github.com/faea726/vbook-extension-maker/releases/latest) vào VSCode
