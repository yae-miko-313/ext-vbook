...
2. Run `node .codex/skills/vbook-extensions/scripts/vbook.js test <ext-dir> <script>.js <args...>` (see the CLI section in `SKILL.md`; it checks `/connect` and logs input/output).
3. Read the result:
...
2. Verify `data` against the shared standard in `references/verify-checklist.md` (fields present + right type, no silent empty/null, real usable URLs, values match the live page, expected array counts, no silent domain move).
