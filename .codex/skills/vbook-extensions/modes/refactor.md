...
2. **Compare against the Phase 0 baseline** — same field set, same/equivalent values, same array counts, same `link`/`cover`/`url` hosts. `code:0` alone is not a pass; the data must match what it produced before (the field-rigor bar is the shared standard in `references/verify-checklist.md`, but here parity with baseline is the pass condition, not just validity).
