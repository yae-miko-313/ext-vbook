...
function obfuscateExtension(extPath) {
  ...
  for (const file of files) {
    ...
    const { success, inputSize, outputSize, error } = obfuscateFile(
      inputPath,
      outputPath,
    );
    ...
    if (success) {
      ...
      console.log(
        `✅ ${file.padEnd(20)} ${inputSize.toString().padStart(7)} B → ${outputSize.toString().padStart(7)} B (${ratio}%)`,
      );
      successCount++;
    } else {
...
if (require.main === module) {
  ...
  if (!extPath) {
    ...
    console.error(
      "Example: node obfuscate.js extensions/novel/hieu05_truyencv",
    );
    process.exit(1);
  }
