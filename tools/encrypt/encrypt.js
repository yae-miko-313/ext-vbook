...
function generateEncryptionKey(metadata) {
  ...
  const hexKey = crypto.createHash("md5").update(payload, "utf8").digest("hex");

  // Step 2: SHA256(hexKey as UTF-8) → 32-byte AES key
  const aesKey = crypto.createHash("sha256").update(hexKey, "utf8").digest();
...
function encryptExtension(extPath) {
  ...
  for (const file of bundleFiles) {
    ...
    const encryptedPath = path.join(
      srcDir,
      file.replace(".bundle.js", ".encrypted.js"),
    );
    ...
    try {
      ...
      console.log(
        `✅ ${file.padEnd(25)} → ${encryptedSize.toString().padStart(7)} B encrypted`,
      );
  ...
  console.log(`\n${"=".repeat(70)}`);
  console.log(
    `✅ Result: ${successCount}/${bundleFiles.length} files encrypted`,
  );
...
if (require.main === module) {
  ...
  if (!extPath) {
    ...
    console.error("Example: node encrypt.js extensions/novel/hieu05_truyencv");
    process.exit(1);
  }
