...
function parseAndNormalizeOptions(workspaceRoot, options = {}) {
  ...
  const author = String(
    options.author || process.env.VBOOK_AUTHOR || "hieu05",
  ).trim();

  const rawSlug = slugifyName(name).replace(/-/g, "_");
  const authorPrefix = `${slugifyName(author)}_`;
  const defaultFolder = rawSlug.startsWith(authorPrefix)
    ? rawSlug
    : `${authorPrefix}${rawSlug}`;
  ...
  const description = String(
    options.description || `Generated extension for ${source}`,
  ).trim();
  ...
  if (!folderName)
    throw new Error("Unable to build target folder name from extension name.");
  if (!ALLOWED_TYPES.includes(type)) {
    throw new Error(
      `Unsupported type: ${type}. Allowed: ${ALLOWED_TYPES.join(", ")}`,
    );
  }
  if (!ALLOWED_LOCALES.includes(locale)) {
    throw new Error(
      `Unsupported locale: ${locale}. Allowed: ${ALLOWED_LOCALES.join(", ")}`,
    );
  }
...
function resolvePluginJsonPath(workspaceRoot, pluginOption) {
  ...
  if (stat.isFile()) {
    if (path.basename(candidate) !== "plugin.json") {
      throw new Error(
        "--plugin must be an extension folder or plugin.json file.",
      );
    }
    return candidate;
  }
...
function scaffoldExtension(workspaceRoot, options = {}) {
  ...
  if (fs.existsSync(config.outputDir) && !config.force) {
    throw new Error(
      `Output directory already exists: ${config.outputDir} (use --force to overwrite)`,
    );
  }
