...
program
  ...
  .action(() => {
    ...
    fork(path.join(__dirname, "vbook.js"), process.argv.slice(2), {
      stdio: "inherit",
    });
  });
...
program
  ...
  .option(
    "--from-reference <path>",
    "Thu muc tham chieu hoc tap",
    ".private/code-reference/_unknown/example",
  )
...
program
  ...
  .option(
    "--plugin <path>",
    "Extension folder or plugin.json path",
    "extensions/novel/hieu05_ntruyen",
  )
...
program
  ...
  .action(async (url, options) => {
    const success = await handleAnalyzeCommand(
      { ...options, url },
      WORKSPACE_ROOT,
    );
    if (!success) process.exitCode = 1;
...
program
  .command("sync-ref")
  .description(
    "Sync community repos and merge extensions into .private/code-reference",
  )
  .option("--json", "Output as JSON")
  .action(async (options) => {
  ...
  .addCommand(
    new Command("debug")
      ...
      .action(async (file, subOptions, ...rest) => {
        ...
        const success = await handleDeviceDebug(
          file,
          subOptions,
          command.parent,
        );
