...
- **Hoạt động**: Tự động sinh ra cấu trúc thư mục chuẩn tại `extensions/{loại}/{tên}` bao gồm `plugin.json` chứa metadata gốc và thư mục `src/` chứa các tệp mã nguồn JS trống.
